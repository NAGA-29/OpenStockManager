...
/**
 * レンタル登録時の端末状態チェック（旧 StoreRentalCartRequest::withValidator 相当）。
 * 販売済み・貸出中・不良の端末を弾く（販売対象外フラグはレンタルでは無視）。
 */
trait ChecksRentableDevices
{
    /**
     * @param array<int, string> $deviceIds 検証対象の端末 ID 一覧
     * @param string $errorKey エラーを追加するフィールド名
     */
    protected function validateRentableDevices(Validator $validator, array $deviceIds, string $errorKey): void
    {
