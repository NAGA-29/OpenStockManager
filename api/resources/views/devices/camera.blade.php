...
@section('main_contents')
    <div>
        @include('component.summary_cards', ['title' => __('カメラ')])
        ...
        <div>
            <div class="row device-toolbar">
                ...
                @include('component.search_form', ['hiddenType' => 'CAM'])
            </div>
            <div class="row device-pagination">
                <div class="col-md-12 col-xl-6">
                    {{ $cam_info_collection->links() }}
                </div>
            </div>
        </div>
        <div class=" bg-white rounded shadow table-responsive text-nowrap">
            <table class="table table-hover">
                ...
                <tbody>
                    @foreach ($cam_info_collection as $device_info)
                        ...
                        <td>
                            <input type="checkbox" class="product-checkbox" id="product-{{ $device_info->device_id }}"
                                ...
                                device-name="{{ $device_info->device_name }}" device-type="{{ $device_info->device_type }}"
