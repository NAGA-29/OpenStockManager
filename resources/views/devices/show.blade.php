    ...
    <div class="card">
        <div class="card-body m-5">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @elseif ($fieldDef->field_type === 'select' && $fieldDef->options)
                                @php
                                    $optLabel = collect($fieldDef->options)->firstWhere('value', $cfVal)['label'] ?? $cfVal;
                                @endphp
                                {{ $optLabel }}
                            @else
                                {{ $cfVal }}
            ...
            <div class='text-center pt-5'>
                <a class="btn btn-outline-dark mr-3"
                    href="{{ route('device.barcode', ['device_id' => $device_info_collection->device_id]) }}"
                    target="_blank">
                    <i class="fas fa-barcode"></i> バーコード印刷
                </a>
                <button type="button" class="btn btn-outline-dark mr-3" data-bs-toggle="modal"
                ...
                @elseif (!$device_info_collection->sale_id && !$device_info_collection->lending_now)
                    <a class="btn btn-outline-dark mr-3"
                        href="{{ url('/device/rental', $device_info_collection->device_id) }}" role="button">貸出</a>
                    <a class="btn btn-outline-dark mr-3"
                        href="{{ url('/device/sales', $device_info_collection->device_id) }}" role="button">販売</a>
                    {{-- sale値なし & rental値あり --}}
                @elseif (!$device_info_collection->sale_id && $device_info_collection->lending_now)
                    <button type="button" class="btn btn-outline-danger mr-3" data-bs-toggle="modal"
